...
|| **T8** | **Tests** — `chart-engine.test.ts` for each Query Config variant (including computed field `removal_m3`); update `chart-tools.test.ts` for config-based creation (both query_config and legacy data modes); `recomputeAllCharts.test.ts` | `src/lib/ai/__tests__/chart-engine.test.ts`, update existing | 3h | ⏳ Deferred |
