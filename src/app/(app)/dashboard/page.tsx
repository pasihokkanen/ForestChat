...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      // Compute total area from compartments (always populated for both CSV and WFS)
      ...
      {stats && stats.count > 0 && (
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4">
            <p className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider">Forests</p>
            <p className="text-2xl font-semibold text-gray-900 dark:text-gray-100 mt-1">
              {stats.count}
            </p>
          </div>
          <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4">
            <p className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider">Total Area</p>
            <p className="text-2xl font-semibold text-gray-900 dark:text-gray-100 mt-1">
      ...
      {hasForests === false && (
        <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-8 text-center mb-6">
          ...
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400 max-w-sm mx-auto">
            Import your forest data to start managing your forest plan with AI.
            You can use a Finnish property ID or upload a CSV file.
          </p>
          <Link
            href="/forest/new"
