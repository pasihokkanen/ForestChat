...
export default function StandPopup({ properties }: StandPopupProps) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-3 min-w-[200px] text-sm text-gray-900">
      <h3 className="font-semibold text-base mb-2 border-b pb-1">
        Stand {stand_id}
      </h3>
      <dl className="grid grid-cols-2 gap-x-3 gap-y-1">
        <dt className="text-gray-500">Main species</dt>
        <dd>{main_species ?? "—"}</dd>

        <dt className="text-gray-500">Development class</dt>
        <dd>{development_class ?? "—"}</dd>

        <dt className="text-gray-500">Site type</dt>
        <dd>{site_type ?? "—"}</dd>

        <dt className="text-gray-500">Area (ha)</dt>
        <dd>{area_ha != null ? area_ha.toFixed(1) : "—"}</dd>

        <dt className="text-gray-500">Age</dt>
        <dd>{age_years != null ? `${age_years} yr` : "—"}</dd>

        <dt className="text-gray-500">Volume (m³)</dt>
