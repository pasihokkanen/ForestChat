...
export function buildSystemPrompt(
  ...
  return [
    ...
    `- Available sources: operations (planned harvests), compartments (stand data), compartment_species (per-species breakdown with species, volume_m3, area_ha), plan_metadata.`,
    `- Available computed fields: removal_m3 (= volume_m3 × removal_pct / 100, requires join with compartments),`,
    `  net_cashflow (= income_eur − cost_eur, auto-computed per operation).`,
    ...
    `- For charts combining income and costs: use TWO value entries, with cost_eur multiplied by -1`,
    `  to show costs below zero. Set y_key2 to the cost field's alias so the chart renders a dual-stack`,
    `  (income above zero, costs below zero).`,
    `- Common query_config templates:`,
    `  * Yearly income only (bar/line/area):`,
    `    { source: "operations", aggregate: [{ group_by: "year" }],`,
    `      values: [{ field: "income_eur", as: "income", fn: "sum" }], sort: { by: "year" } }`,
    `  * Cumulative (running total) income for area charts: same as yearly but add`,
    `    cumulative: true to the value entry. This converts yearly sums to running totals:`,
    `    { source: "operations", aggregate: [{ group_by: "year" }],`,
    `      values: [{ field: "income_eur", as: "income", fn: "sum", cumulative: true }],`,
    `      sort: { by: "year" } }`,
    `    When the user says "cumulative" or "running total", always set cumulative: true.`,
    `  * Yearly income AND costs (stacked_bar with dual-stack):`,
    ...
    `      sort: { by: "year" } }`,
    `      → Pass x_key: "year", y_key: "income", y_key2: "cost", color_key: "type"`,
    `  * Total income by operation type (bar/horizontal_bar):`,
    `    { source: "operations", aggregate: [{ group_by: "type" }],`,
    ...
    `  * Species area distribution (dominant species only — don't use for "area by species"):`,
    `    { source: "compartments", aggregate: [{ group_by: "main_species" }],`,
    `      values: [{ field: "area_ha", as: "total_ha", fn: "sum" }] }`,
    `  * Multi-species area/volume (USE THIS for "area by species" or "volume by species"):`,
    `    { source: "compartment_species", aggregate: [{ group_by: "species" }],`,
    `      values: [{ field: "area_ha", as: "total_ha", fn: "sum" }] }`,
    `    or for volume: [{ field: "volume_m3", as: "total_m3", fn: "sum" }]`,
    `    This is the PREFERRED source — it includes ALL species per stand, not just the dominant one.`,
    `    For pie/donut charts, also pass name_key: "species" to label the slices.`,
    ...
    `      values: [{ field: "income_eur", as: "income", fn: "sum" }], sort: { by: "income", dir: "desc" } }`,
    `  * Waterfall chart (yearly net cashflow = income − costs):`,
    `    Uses the built-in net_cashflow computed field for automatic income − cost:`,
    `    { source: "operations", aggregate: [{ group_by: "year" }],`,
    `      values: [{ field: "net_cashflow", as: "net", fn: "sum" }],`,
    `      sort: { by: "year" } }`,
    `    Pass x_key: "year", y_key: "net". Green bars = net gain, red = net loss.`,
    `    For any "cashflow", "net", or "income minus costs" request, use net_cashflow.`,
    `  * Scatter plot (stand-level two-variable comparison, e.g. "age vs volume"):`,
    `    Group by stand_id so each stand becomes one dot. Use TWO value entries — the`,
    `    FIRST becomes the x-axis (via x_key), the SECOND becomes the y-axis (via y_key):`,
    `    { source: "compartments", aggregate: [{ group_by: "stand_id" }],`,
    `      values: [`,
    `        { field: "age_years", as: "age", fn: "avg" },`,
    `        { field: "volume_m3", as: "volume", fn: "avg" }`,
    `      ] }`,
    `    Pass x_key: "age", y_key: "volume". For "diameter vs height": avg_diameter and`,
    `    avg_height. For "age vs growth": age_years and growth_m3_per_ha.`,
    `    IMPORTANT: Scatter plot data is fetched AUTOMATICALLY by the query_config.`,
    `    NEVER call search_stands before create_chart — just call create_chart directly!`,
