...
export function buildSystemPrompt(
  ...
  return [
    ...
    `- Add cumulative: true on a value entry to convert sums to running totals.`,
    `- For pie/donut: also pass name_key to label slices (usually the group_by field).`,
    ``,
    ...
    ``,
    `NON-OBVIOUS PATTERNS (standard groupings are straightforward from the schema):`,
    ``,
    `  Stacked bar income+costs: TWO values, cost with multiply: -1, TWO group_by`,
    `    (year + type), pass color_key: "type" and y_key2: "cost".`,
    ``,
    `  Join (for removal_m3 or filtering by compartment fields):`,
    `    join: { table: "compartments", on: "compartment_id", fields: ["volume_m3"] }`,
    `    Refer to joined fields as "comp.fieldname" in group_by, e.g. "comp.main_species".`,
    ``,
    `  Filters: { development_class: "Uudistuskypsä metsikkö" } — filters by any column.`,
    `    Use on compartments source for filtered-by-dev-class species charts.`,
    ``,
    `  Waterfall: use net_cashflow computed field, group by year, type: "waterfall".`,
    ``,
    `  Scatter: group by stand_id, TWO value entries → first is x_key, second is y_key.`,
    `    NEVER call search_stands before create_chart — data is fetched automatically.`,
    ``,
    `- Common group_by fields: year, type, main_species, species, development_class, stand_id.`,
    `- Common values (.field → .as): income_eur→income, cost_eur→cost, area_ha→total_ha,`,
    `  volume_m3→total_m3, age_years→age, growth_m3_per_ha→growth, removal_m3→volume.`,
    `- Always include sort: { by: "year" } when grouping by year.`,
    `- Chart data auto-updates on plan changes. Only skip query_config for static data.`,
  ]
    .filter(Boolean)
    .join("\n");
}
