...
export function buildSystemPrompt(
  ...
  return [
    ...
    `- Available sources: operations (planned harvests), compartments (stand data), compartment_species (per-species breakdown with puulaji, volume_m3, area_ha), plan_metadata.`,
    ...
    `  * Total income by operation type (bar/horizontal_bar — single group_by):`,
    ...
    `    { source: "compartment_species", aggregate: [{ group_by: "puulaji" }],`,
    `      values: [{ field: "area_ha", as: "total_ha", fn: "sum" }] }`,
